...
sudo iptables -t nat -A PREROUTING -i eth0 -p tcp --dport $constant_port -m conntrack --ctstate NEW -j REDIRECT --to-port $app_port  || (echo "ERROR: redirect $constant_port to $app_port failed" ; exit 1)
